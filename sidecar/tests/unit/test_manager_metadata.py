...
from core.types import AgentConfig, AgentID
...
async def test_get_agents_metadata():
    ...
    metadata: list[AgentConfig] = manager.get_agents_metadata()

    assert isinstance(metadata, list)
    assert len(metadata) >= 2

    ids = [m["agent_id"] for m in metadata]
    assert AgentID.TEACHER.value in ids
    assert AgentID.REVIEWER.value in ids

    for item in metadata:
        assert all(key in item for key in ["agent_id", "name", "description", "icon"])
